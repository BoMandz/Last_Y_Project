...
private:
    ...
    void writeToFile(const ErrorRecord& record) {
        ...
        try {
            ...
            if (consoleOutputEnabled) {
                conHandler.The_Output_New_Line("Failed to write to log file: " + logFilePath);
            }
        }
    }
    ...
    void writeToConsole(const ErrorRecord& record) {
        ...
        conHandler.The_Output_New_Line(formattedMessage);
    }
    ...
    void handleFatalError(const std::string& message) {
        ...
        if (consoleOutputEnabled) {
            conHandler.The_Output_New_Line("FATAL ERROR: Performing cleanup before exit...");
        }
        ...
        for (auto& cleanup : cleanupFunctions) {
            try {
                ...
                if (consoleOutputEnabled) {
                    conHandler.The_Output_New_Line("Exception during cleanup function execution");
                }
            }
        }
        ...
        // Final log message
        if (consoleOutputEnabled) {
            conHandler.The_Output_New_Line("Cleanup complete. Application will exit.");
        }
